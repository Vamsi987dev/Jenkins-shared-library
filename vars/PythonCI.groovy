            ...
            stage('Prepare Version') {
                steps {
                    script {
                        ...
                        // Try multiple version sources (real-world flexibility)
                        if (fileExists('package.json')) {
                            def packageJson = readJSON file: 'package.json'
                            env.IMAGE_TAG = packageJson.version
                        } 
                        else if (fileExists('pyproject.toml')) {
                            def version = sh(script: "grep '^version' pyproject.toml | cut -d '\"' -f2", returnStdout: true).trim()
                            env.IMAGE_TAG = version
                        } 
                        else {
                            error "No version source found (package.json or pyproject.toml)"
                        }

                        if (!env.IMAGE_TAG?.trim()) {
                            error "Version is empty. Failing pipeline."
                        }
